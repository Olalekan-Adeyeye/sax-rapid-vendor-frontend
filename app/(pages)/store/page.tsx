...
export default function StoreProfile() {
  ...
  const upgradeMutation = useMutation({
    ...
    onSuccess: () => {
      toast("Success", "Account upgraded to Business!", "success");
    ...
    onError: (err: unknown) => {
      toast("Upgrade Failed", getErrorMessage(err), "error");
    },
  });
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12">
      ...
      <Modal
        ...
        subtitle="Submit your company details to unlock business features"
        icon={Building2}
      >
        <form
          onSubmit={(e) => {
        ...
        >
          ...
          <div className="space-y-2">
            <label className="block text-sm font-bold text-gray-400">
              Business Document (CAC certificate, etc.)
            </label>
            {docUrl ? (
              <div className="flex items-center gap-3 bg-green-50 border border-green-100 rounded p-3">
                <Check size={16} className="text-green-600 shrink-0" />
                <span className="text-xs font-bold text-green-700 truncate flex-1">
                  Document uploaded
                </span>
                <button
                  type="button"
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 border-2 border-dashed border-gray-200 rounded p-6 text-gray-400 hover:border-gray-400 hover:text-gray-600 transition-colors"
              >
                {docUploading ? (
            ...
            <input
              ...
              onChange={async (e) => {
                ...
                try {
                  const res = await uploadFile(file, "vendor-documents");
                  if (res?.url) setDocUrl(res.url);
                } catch {
                  toast("Upload Failed", "Could not upload document", "error");
                } finally {
          ...
          <div className="flex gap-4 pt-2">
            ...
            <Button
              type="submit"
              fullWidth
              disabled={!companyName || !regNumber || !docUrl}
