        ...
        if (galleryDirtyRef.current) {
          ...
          try {
            ...
            if (results.uploaded.length !== localImages.length) {
              throw new Error(
                `Only ${results.uploaded.length} of ${localImages.length} images uploaded`,
              );
            }
            galleryUrlsRef.current = localImages
      ...
      if (variationImagesToUpload.length > 0 && varDirtyRef.current) {
        ...
        try {
          ...
          if (results.uploaded.length !== variationImagesToUpload.length) {
            throw new Error(
              `Only ${results.uploaded.length} of ${variationImagesToUpload.length} variation images uploaded`,
            );
          }
