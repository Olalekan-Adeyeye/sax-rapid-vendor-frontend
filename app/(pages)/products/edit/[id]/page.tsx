  ...
  const fetchProduct = useCallback(async () => {
    ...
    try {
      ...
      reset({
        ...
        saleStartDate: product.salePriceStartDate
          ? product.salePriceStartDate.split("T")[0]
          : "",
      ...
      if (isVariable) {
        ...
        const mappedVariations = product.variations?.map((v) => {
          ...
          return {
            ...
            saleStartDate: v.salePriceStartDate
              ? v.salePriceStartDate.split("T")[0]
              : "",
            saleEndDate: v.salePriceEndDate
              ? v.salePriceEndDate.split("T")[0]
              : "",
      ...
      if (newFiles.length > 0 && galleryDirtyRef.current) {
        ...
        try {
          ...
          if (results.uploaded.length !== newFiles.length) {
            throw new Error(
              `Only ${results.uploaded.length} of ${newFiles.length} images uploaded`,
            );
          }
          galleryUrlsRef.current = newFiles
            .map((_, i) => results.uploaded[i]?.url || "")
            .filter(Boolean);
      ...
      if (variationsToUpload.length > 0 && varDirty) {
        ...
        try {
          ...
          if (results.uploaded.length !== variationsToUpload.length) {
            throw new Error(
              `Only ${results.uploaded.length} of ${variationsToUpload.length} variation images uploaded`,
            );
          }
