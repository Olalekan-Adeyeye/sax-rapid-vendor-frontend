				...
				<div className="overflow-x-auto">
					<table className="w-full text-left min-w-250">
						<thead className="bg-gray-50 border-b border-gray-100">
							<tr>
								{["Product", "Category", "Base Price", "SKU", "Stock", "Status", ""].map((th) => (
						...
						<tbody className="divide-y divide-gray-50">
							{[
								...
								<tr key={i} className="hover:bg-gray-50/50 transition-colors group">
									...
									<td className="px-8 py-5 text-xs font-bold text-gray-500 uppercase">{product.cat}</td>
									<td className="px-8 py-5 text-xs font-black text-black">{product.price}</td>
									<td className="px-8 py-5 text-xs font-bold text-gray-400 uppercase tracking-widest">{product.sku}</td>
									<td className="px-8 py-5">
										<div className="flex flex-col gap-1.5 min-w-25">
											<div className="flex items-center justify-between text-[10px] font-black uppercase">
												<span className={product.stock === 0 ? "text-red-500" : "text-black"}>{product.stock} left</span>
												<span className="text-gray-300">/ 200</span>
											</div>
											<div className="h-1 bg-gray-50 rounded-full overflow-hidden">
												<div className={`h-full rounded-full ${product.stock === 0 ? 'bg-red-500' : 'bg-gold'}`} style={{ width: `${(product.stock / 200) * 100}%` }} />
											</div>
										</div>
									</td>
									<td className="px-8 py-5">
									...
									<td className="px-8 py-5 text-right">
										<div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
											<button className="w-9 h-9 rounded bg-white border border-gray-100 flex items-center justify-center text-gray-400 hover:text-black hover:border-gold transition-all"><Edit size={14} /></button>
											<button className="w-9 h-9 rounded bg-white border border-gray-100 flex items-center justify-center text-gray-400 hover:text-red-500 hover:border-red-500 transition-all"><Trash2 size={14} /></button>
											<button className="w-9 h-9 rounded bg-white border border-gray-100 flex items-center justify-center text-gray-400 hover:text-gold transition-all"><MoreVertical size={14} /></button>
										</div>
									</td>
								</tr>
							))}
