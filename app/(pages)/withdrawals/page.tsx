...
import { ArrowDownCircle, MoreVertical, CreditCard, Banknote, Clock, CheckCircle } from "lucide-react";
		...
		<div className="space-y-10">
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
				{[
					...
					<div key={i} className="bg-white border border-gray-100 rounded p-6 lg:p-8 hover:border-gold transition-all group">
						...
						<p className="text-xs font-bold text-gray-500 mb-1 transition-colors">{stat.label}</p>
			...
			<div className="bg-white border border-gray-100 rounded overflow-hidden">
				<div className="p-8 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-6">
					...
					<button className="text-xs font-bold text-gold hover:underline transition-all">+ Add Bank Account</button>
				</div>
				<div className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
					{[
						...
						<div key={i} className="bg-gray-50/50 border border-gray-100 rounded p-6 flex items-center justify-between group hover:border-black hover:bg-white transition-all">
							<div className="flex items-center gap-6">
								...
								<div>
									<h5 className="text-sm font-bold text-black">{method.bank}</h5>
									<p className="text-xs font-bold text-gray-400 mt-1">{method.account} · {method.holder}</p>
								</div>
							</div>
							<div className="flex items-center gap-4">
								<span className="text-xs font-bold px-2.5 py-1.5 rounded bg-white border border-gray-100 text-gray-400 group-hover:border-black group-hover:text-black transition-all">{method.status}</span>
						...
						<thead className="bg-gray-50 border-b border-gray-100">
							<tr>
								{["Payout ID", "Method", "Amount", "Reference", "Status", "Date"].map((th) => (
									<th key={th} className="px-8 py-5 text-xs font-bold text-gray-400">{th}</th>
								))}
							</tr>
						</thead>
						<tbody className="divide-y divide-gray-50">
							{[
								...
								<tr key={i} className="hover:bg-gray-50/50 transition-colors">
									...
									<td className="px-8 py-5 text-xs font-bold text-black">{p.bank}</td>
									<td className="px-8 py-5 text-xs font-bold text-black">{p.amount}</td>
									<td className="px-8 py-5 text-sm font-bold text-gray-400">{p.ref}</td>
									...
									<td className="px-8 py-5 text-sm font-bold text-gray-400">{p.date}</td>
								</tr>
							))}
						</tbody>
					</table>
